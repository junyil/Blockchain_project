...
it('can add the star name and star symbol properly', async() => {
    ...
    let instance = await StarNotary.deployed();
    let starId = 6;
    await instance.createStar('Shiba', starId, {from: accounts[0]});
    assert.equal(await instance.tokenIdToStarInfo.call(starId), 'Shiba');
    assert.equal(await instance.symbol(), 'SBS');
    assert.equal(await instance.name(), 'ShibaInuStar');
});
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar('Golden', starId1, {from: accounts[0]});
    await instance.createStar('Yugi', starId2, {from: accounts[1]});
...
it('lets a user transfer a star', async() => {
    ...
    let instance = await StarNotary.deployed();
    let starId = 10;
    await instance.createStar('Jackson', starId, {from: accounts[0]});
